...
BMsummary=summary(body.mass.data)
print(BMsummary)
...
print(BMdf)
test = colnames(BMdf) <- c("Species", "BMmean", "BMsd")
...
#BMmean2=aggregate(x=data.na.out2$Body.Mass, by=list(data.na.out2$Species), FUN= c(mean,sd))
BMmean2=aggregate(x=data.na.out2$Body.Mass, by=list(data.na.out2$Species), FUN= mean)
BMsd2=aggregate(x=data.na.out2$Body.Mass, by=list(data.na.out2$Species), FUN= sd)
...
# Sp.group=group_by(data.na.out2, data.na.out2$Species)
# print(Sp.group)


BMsum=summarize(Sp.group, 
                BMmean3=mean($Body.Mass..g.),
                BMsd3=sd(Sp.group$Body.Mass..g.)
              )
print(BMsum)

geom_errorbar(data = BMdf, aes(ymin=BMmean, ymax=BMmean), width=0.01, col="black")
...
plot5=ggplot(body.mass.data, aes())
